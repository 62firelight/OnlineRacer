...
Deno.serve({
    ...
    async handler(request) {
        ...
        socket.onmessage = (event) => {clientWebSocketMessage.call(socket, JSON.parse(event.data))};

        socket.onclose = () => { 
            clientWebSocketMessage.call(socket, {type:"player_disconnected"}); // Send msg to indicate disconnection to close any associated server state
            clientWebSocketDisconnected();
        }
